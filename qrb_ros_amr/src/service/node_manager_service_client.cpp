  ...
  : Node("node_manager", options), amr_manager_(amr_manager)
{
  ...
  client_list_.push_back(this->create_client<ChangeState>("/navigation_path_service_server/change_state"));
  client_list_.push_back(this->create_client<ChangeState>("/virtual_path_service_server/change_state"));
...
void NodeManagerServiceClient::send_request(
  const ChangeState::Request::SharedPtr request,
